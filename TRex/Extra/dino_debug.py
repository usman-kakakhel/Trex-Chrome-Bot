...
class Control(object):
		...
		while not self.done:
			...
			if not self.paused:
				...
				if self.dino.state != DINO_STATES['dead']:
					...
					if bird_count < 1 and random.randrange(0, 15) == 12 and self.hud.score > 30:
	...
	def update(self):
		...
		if self.tempcounter % 10 == 0:
			type,x,y = get_nearest_enemy(self.dino, self.cactus_objects, self.bird_objects)
			print("type: " + str(type) + " x: " + str(x) + " y: " + str(y))
			pause()
			self.tempcounter = 1
		else:
			self.tempcounter += 1
